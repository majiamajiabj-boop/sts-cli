...
FRAMES = json.loads((Path(__file__).resolve().parents[1] / "test_fixtures/sustained_damage_frames.json").read_text(encoding="utf-8"))
...
class SustainedDamageTests(unittest.TestCase):
    ...
    def test_retained_block_growth_and_recurring_poison_keep_credit(self):
        from tests.test_live_macro_decisions import make_card, CardType
        agent = agent_for("af024a34")
        deck = list(agent.game.deck) + [
