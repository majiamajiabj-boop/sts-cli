...
public class GameStateListener {
    /** Read-only readiness for manual-player snapshots. */
    public static boolean isAdvisorReady() {
        if (!CommandExecutor.isInDungeon()) return false;
        if (blocked || AbstractDungeon.isFadingIn || AbstractDungeon.isFadingOut) return false;
        boolean selection = AbstractDungeon.screen.name().equals("HAND_SELECT")
                || AbstractDungeon.screen.name().equals("GRID");
        if ((!selection && !isActionManagerReadyForCommand()) || hasPendingDungeonEffects()) return false;
        if (AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT
                && !AbstractDungeon.isScreenUp
                && (!myTurn || AbstractDungeon.player.endTurnQueued)) return false;
    ...
    private static final int COMBAT_SCREEN_CLOSE_SETTLE_UPDATES = 4;
    private static int combatScreenCloseUpdatesRemaining = 0;
    ...
    public static void registerCommandExecution() {
        waitingForCommand = false;
        // Screen/phase detectors normally publish much sooner.  Keep a
        // bounded fallback so an accepted command can never wait forever when
        // a base-game screen mutates without tripping those detectors.
        if (timeout <= 0) {
            // The external controller may be waiting for the first stable
            // frame after a fast hand/grid selection.  A long fallback makes
            // an already-settled command look like a receipt timeout when
            // the mutation patch did not register a state change.  Poll the
            // same stability predicates sooner; hasDungeonStateChanged()
            // still refuses to publish until the action is actually ready.
            timeout = 30;
        }
    }
    ...
    public static void resetStateVariables() {
        ...
        waitingForCommand = false;
        waitOneUpdate = false;
        combatScreenCloseUpdatesRemaining = 0;
    }

    /**
     * Visual obtain effects are part of the authoritative transaction.
     *
     * Several events replace their buttons with "Leave" before queued
     * ShowCardAndObtainEffect instances have added cards to the master deck.
     * Publishing that intermediate state lets an external controller leave
     * the room and cancel rewards that were already paid for.  Check both the
     * active and next-frame queues because the dungeon moves effects between
     * them during update.
     */
    private static boolean isTransactionalDungeonEffect(Object effect) {
        if (effect == null) {
            return false;
        }
        String name = effect.getClass().getSimpleName();
        return name.contains("Obtain")
                || name.equals("PurgeCardEffect")
                || name.equals("GainPennyEffect")
                || name.equals("RainingGoldEffect");
    }

    private static boolean containsTransactionalDungeonEffect(Iterable<?> effects) {
        for (Object effect : effects) {
            if (isTransactionalDungeonEffect(effect)) {
                return true;
            }
        }
        return false;
    ...
    private static boolean hasPendingDungeonEffects() {
        // AbstractDungeon's effect lists also contain ambient particles that
        // may be replenished forever (map circles, campfire smoke, etc.).
        // Waiting for every visual effect would deadlock valid decisions, so
        // gate only effects whose lifetime can still mutate persistent state.
        return containsTransactionalDungeonEffect(AbstractDungeon.effectList)
                || containsTransactionalDungeonEffect(AbstractDungeon.effectsQueue)
                || containsTransactionalDungeonEffect(AbstractDungeon.topLevelEffects)
                || containsTransactionalDungeonEffect(AbstractDungeon.topLevelEffectsQueue);
    }

    /**
     * A room transition is not a safe decision surface while the action
     * manager is still resolving the previous room's actions.  In particular,
     * a combat reward can become visible one frame before the final card
     * action has finished; publishing that frame lets the controller proceed
     * into the next room while the base game is still using the old card.
     */
    private static boolean isActionManagerReadyForCommand() {
        if (AbstractDungeon.actionManager == null) {
            return true;
        }
        return AbstractDungeon.actionManager.phase.equals(GameActionManager.Phase.WAITING_ON_USER)
    ...
    private static boolean hasDungeonStateChanged() {
        ...
        // Outside combat, rewards and other event mutations commonly settle
        // through dungeon effects rather than the action manager.  Never
        // advertise a new decision surface while those effects are pending.
        if (!inCombat && hasPendingDungeonEffects()) {
            return false;
        }
        // Room/screen transitions can be visible before the last combat
        // action has settled.  Do not expose a non-combat command surface
        // until the action manager has returned to user input with every
        // queue drained; otherwise a proceed/map command can race a stale
        // card action and crash the base game during the next room update.
        ...
        if (newScreen != previousScreen || newScreenUp != previousScreenUp || newPhase != previousPhase) {
            if (inCombat) {
                // Hand-select actions such as Gambling Chip finish their
                // discard after the screen closes, then enqueue replacement
                // draws on later dungeon updates.  The action manager can be
                // momentarily empty between those two stages, so an
                // emptiness check after only one update is not a settlement
                // proof.  Consume a small bounded run of updates before
                // exposing the normal combat surface.
                if (previousScreenUp && !newScreenUp) {
                    combatScreenCloseUpdatesRemaining =
                            COMBAT_SCREEN_CLOSE_SETTLE_UPDATES;
                ...
                if (newScreenUp) {
                    // A chained selection (e.g. Burst + Survivor) needs input
                    // while its action queue is occupied. The prior screen's
                    // close barrier must not block subsequent selection changes.
                    // Closing this new screen will establish a fresh barrier.
                    combatScreenCloseUpdatesRemaining = 0;
                    return true;
                }
        ...
        } else if (combatScreenCloseUpdatesRemaining > 0) {
            if (combatScreenCloseUpdatesRemaining > 1) {
                combatScreenCloseUpdatesRemaining -= 1;
                return false;
            }
            if (!isActionManagerReadyForCommand()) {
                return false;
            }
            combatScreenCloseUpdatesRemaining = 0;
            return true;
        ...
        if (timeout > 0) {
            timeout -= 1;
            if(timeout == 0) {
                boolean actionQueuesEmpty = AbstractDungeon.actionManager.phase.equals(GameActionManager.Phase.WAITING_ON_USER)
                        && AbstractDungeon.actionManager.preTurnActions.isEmpty()
                        && AbstractDungeon.actionManager.actions.isEmpty()
                        && AbstractDungeon.actionManager.cardQueue.isEmpty();
                boolean stableForCommand = (!inCombat && !hasPendingDungeonEffects())
                        || (inCombat && newScreenUp)
                        || (myTurn && !AbstractDungeon.player.endTurnQueued && actionQueuesEmpty);
                if (stableForCommand) {
                    return true;
                }
                // Keep polling without publishing an intermediate state that
                // cannot legally accept the controller's next command.
                timeout = 1;
            }
        }
        return false;
    ...
    public static boolean checkForDungeonStateChange() {
        boolean stateChange = false;
        // Once a stable state has been published, there is nothing new to
        // discover until a command or one of the mutation patches marks the
        // listener dirty.  Avoid walking room/monster/action state on every
        // rendered frame while the external planner is thinking.
        if (waitingForCommand && !externalChange && timeout <= 0) {
            return false;
        }
