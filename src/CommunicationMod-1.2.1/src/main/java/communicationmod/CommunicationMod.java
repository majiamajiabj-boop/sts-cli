...
public class CommunicationMod implements PostInitializeSubscriber, PostUpdateSubscriber, PostDungeonUpdateSubscriber, PreUpdateSubscriber, OnStateChangeSubscriber {
    ...
    public CommunicationMod(){
        ...
        if(!AdvisorSnapshot.enabled() && getRunOnGameStartOption()) {
    ...
    public void receivePostUpdate() {
        if (AdvisorSnapshot.enabled()) AdvisorSnapshot.update();
    ...
    private static String[] getSubprocessCommand() {
        ...
        String encoded = communicationConfig.getString("commandJson");
        if (encoded != null && !encoded.trim().isEmpty()) {
            return new Gson().fromJson(encoded, String[].class);
        }
    ...
    private boolean startExternalProcess() {
        if (AdvisorSnapshot.enabled()) return false;
        if(readThread != null) {
            readThread.interrupt();
        }
        if(writeThread != null) {
