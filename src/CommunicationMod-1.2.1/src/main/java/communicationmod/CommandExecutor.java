...
public class CommandExecutor {
    ...
    public static boolean executeCommand(String command) throws InvalidCommandException {
        if (AdvisorSnapshot.enabled()) throw new InvalidCommandException("Read-only advisor mode rejects commands");
    ...
    public static boolean isResumeCommandAvailable() {
        return !isInDungeon()
    ...
    private static void executeStartCommand(String[] tokens) throws InvalidCommandException {
        ...
        // Structured autonomous runs never rely on animation timing.  Fast
        // mode shortens the menu fade and action animations while preserving
        // the exact game-state transitions consumed by CommunicationMod.
    ...
    private static void executeResumeCommand(String[] tokens) throws InvalidCommandException {
        if (tokens.length != 2) {
            throw new InvalidCommandException(tokens, InvalidCommandException.InvalidCommandFormat.MISSING_ARGUMENT);
        }
        AbstractPlayer.PlayerClass selectedClass = null;
        ...
        AbstractPlayer selectedPlayer = CardCrawlGame.characterManager.getCharacter(selectedClass);
        if (selectedPlayer == null || !selectedPlayer.saveFileExists()) {
            throw new InvalidCommandException("No autosave exists for selected character: " + selectedClass.name());
        }

        // Mirror the base game's Continue button, but bind the character
        // explicitly instead of CharacterManager.loadChosenCharacter(), which
        // otherwise selects the first autosave (Ironclad) when several exist.
        CardCrawlGame.characterManager.setChosenCharacter(selectedClass);
        CardCrawlGame.chosenCharacter = selectedClass;
        Settings.FAST_MODE = true;
