...
class Player(Character):
    ...
    def __init__(self, max_hp, current_hp=None, block=0, energy=0):
        ...
        self.energy = energy
        self.orbs = []
        # Older CommunicationMod payloads did not expose the capacity. Keep
        # None distinct from zero so planners can retain their compatibility
        # fallback without overwriting an authoritative reduced slot count.
        self.max_orbs = None
    ...
    def from_json(cls, json_object):
        ...
        player.max_orbs = json_object.get("max_orbs")
        return player
