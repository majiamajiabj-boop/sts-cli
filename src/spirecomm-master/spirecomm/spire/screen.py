...
class EventOption:
    ...
    def __init__(
        self, text, label, disabled=False, choice_index=None, card=None,
        neow_contract=None, original_button_index=None,
        event_contract=None,
    ):
        self.text = text
        self.label = label
        ...
        # ``choice_index`` is compressed over enabled buttons for CHOOSE;
        # this is the stable index in the game's unfiltered option container.
        self.original_button_index = original_button_index
        self.card = card
        # CommunicationMod exposes the base-game Neow enums and parameters.
        # Preserve them verbatim for the strategy layer; localized text is
        # display-only and cannot be an authoritative mechanism identity.
        self.neow_contract = (
            dict(neow_contract) if isinstance(neow_contract, dict) else None
        )
        self.event_contract = (
            dict(event_contract)
    ...
    def from_json(cls, json_object):
        ...
        original_button_index = json_object.get(
            "original_button_index", None
        )
        card = json_object.get("card")
        neow_contract = json_object.get("neow_contract")
        event_contract = json_object.get("event_contract")
        if card is not None:
            card = Card.from_json(card)
        return cls(
            text, label, disabled, choice_index, card, neow_contract,
...
class EventScreen(Screen):
    ...
    def __init__(
        self, name, event_id, body_text="", *, event_class=None,
        event_stage=None, screen_num=None,
    ):
        super().__init__()
        self.event_name = name
        self.event_id = event_id
        self.body_text = body_text
        # A small audited base-game event set exposes exact reflected
        # progress.  Keep it on the parsed screen so action binding and the
        # strategy producer use the same stage evidence as the raw protocol.
    ...
    def from_json(cls, json_object):
        event = cls(
            json_object["event_name"],
            json_object["event_id"],
            json_object["body_text"],
            event_class=json_object.get("event_class"),
            event_stage=json_object.get("event_stage"),
            screen_num=json_object.get("screen_num"),
        )
...
class GridSelectScreen(Screen):
    ...
    def __init__(self, cards, selected_cards, num_cards, any_number, confirm_up, for_upgrade, for_transform, for_purge, parent_choice_context=None):
        super().__init__()
        self.cards = cards
        ...
        self.parent_choice_context = (
            dict(parent_choice_context)
            if isinstance(parent_choice_context, dict) else None
        )
    ...
    def from_json(cls, json_object):
        ...
        parent_choice_context = json_object.get("parent_choice_context")
        return cls(cards, selected_cards, num_cards, any_number, confirm_up, for_upgrade, for_transform, for_purge, parent_choice_context)
